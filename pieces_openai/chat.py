...
models = pieces_client.models_api.models_snapshot()

for model in models.iterable:
    if model.name == pieces_client.model_name:
        tokens = model.max_tokens.total
...
class ChatCompletionRequest(BaseModel):
    model: Optional[str] = pieces_client.model_name
    messages: List[Message]
    max_tokens: Optional[int] = tokens
...
async def _resp_async_generator(text_resp: str):
    ...
    for i, token in enumerate(tokens):
        chunk = {
            ...
            "created": time.time(),
            "model": pieces_client.model_name,
